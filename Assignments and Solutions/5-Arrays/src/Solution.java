 ...
 */
    ...
    public int findMaxValueOfEquation(int[][] points, int k) {
        PriorityQueue<Pair<Integer, Integer>> maxHeap = new PriorityQueue<>((x, y) -> y.getKey() - x.getKey());
        int max = Integer.MIN_VALUE;
...
public boolean findRotation(int[][] mat, int[][] target) {
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            res[i][j] = mat[size - 1 - j][i];
        }
    }

    int[] res2[] = new int[size][size];
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            res2[i][j] = res[size - 1 - j][i];
        }
    }

    int[] res3[] = new int[size][size];
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            res3[i][j] = res2[size - 1 - j][i];
        }
    }
